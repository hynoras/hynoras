# 👋 Hi, I'm Quang

Fullstack Engineer with stronger frontend expertise and practical backend + DevOps knowledge.
I build modern web applications, scalable APIs, and production-oriented systems while continuously learning infrastructure and software architecture.

Graduated from the University of Information Technology in Vietnam.

Currently exploring:

* DevOps & deployment workflows
* System design & scalable architectures
* Building production-grade applications

---

## 🚀 About Me

* 💻 Fullstack engineer focused on modern web technologies
* 🎨 Stronger on frontend engineering and UI architecture
* ⚙️ Experienced building APIs and backend services with Golang & Python
* 🐳 Learning infrastructure, CI/CD pipelines, and deployment workflows
* 🧪 Building side projects that combine engineering, product thinking, and scalability

---

# 🛠 Tech Stack

## Frontend

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge\&logo=react)
![Next JS](https://img.shields.io/badge/Next.js-000000?style=for-the-badge\&logo=next.js)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge\&logo=typescript)
![TailwindCSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=for-the-badge\&logo=tailwind-css)
![TanStack Query](https://img.shields.io/badge/TanStack_Query-FF4154?style=for-the-badge\&logo=react-query)
![Redux](https://img.shields.io/badge/Redux-764ABC?style=for-the-badge\&logo=redux)

## Backend

![Golang](https://img.shields.io/badge/Golang-00ADD8?style=for-the-badge\&logo=go)
![Gin](https://img.shields.io/badge/Gin-008ECF?style=for-the-badge)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge\&logo=python)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge\&logo=fastapi)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge\&logo=flask)

## Database & Cache

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge\&logo=postgresql)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge\&logo=mysql)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge\&logo=mongodb)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge\&logo=redis)

## DevOps

![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge\&logo=docker)
![GitLab CI/CD](https://img.shields.io/badge/GitLab_CI/CD-FC6D26?style=for-the-badge\&logo=gitlab)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=for-the-badge\&logo=nginx)

## Tools

![Cursor](https://img.shields.io/badge/Cursor-000000?style=for-the-badge)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge\&logo=figma)

---

# 📌 Featured Projects

## 🧪 Noheir

A chemistry-focused fullstack platform for managing experiments, structured compound data, and laboratory workflows.

**Tech Stack:** Next.js, Golang, Gin, PostgreSQL, Redis, RabbitMQ

Highlights:

* Fullstack architecture with separated frontend/backend
* Structured relational database design
* CRUD workflows for chemistry-related operations
* Production-oriented backend organization

---

## 🎓 Education Management Platform

A fullstack education management system built with separate React and Spring Boot applications, focused on handling educational workflows and backend-driven business logic.

**Tech Stack:** React, Spring Boot, Java, REST API

Highlights:

* Built frontend and backend as separate applications
* Developed RESTful APIs using Spring Boot
* Implemented frontend integration with backend services
* Structured project organization for scalable feature development
* Explored enterprise-oriented backend architecture patterns

---

## ❤️ Heart Attack Detection

A machine learning-powered application for predicting heart attack likelihood using medical data.

**Tech Stack:** Flutter, FastAPI, Logistic Regression

Highlights:

* Trained Logistic Regression prediction model
* Mobile application + admin dashboard architecture
* Data-driven healthcare prediction workflow

---

# 📊 GitHub Stats

![GitHub Streak](https://streak-stats.demolab.com?user=hynoras&theme=tokyonight)

---

# 📈 Contribution Graph

![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=hynoras&theme=tokyo-night)

---

# 📫 Contact

* 📧 Email: `quangquachvinh0@gmail.com`
* 💼 LinkedIn: [LinkedIn Profile](https://www.linkedin.com/in/qvq1/?utm_source=chatgpt.com)

---

# ⚡ Currently Learning

* Kubernetes & infrastructure orchestration
* Production-grade CI/CD workflows
* System design patterns
* Scalable backend architecture
